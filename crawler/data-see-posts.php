<?php
	...
	<input id = 'moveon' type="checkbox" checked>scan likes to the database</input><br>
	...
	<?php	
		...
		if (isset($_SESSION['facebook_access_token'])) {
			echo '<table>';
			$query = 'SELECT * FROM project_posts LIMIT ' .$start. ' , ' .$limit ;
			...
			while($row = mysql_fetch_assoc($result)) {
				...
				<tr>
					...
					<td class="getLikes" id="likesGetter_<?=$row['id']?>">waiting for results...</td>
				</tr>
				<?php
			}
			echo '</table>';
		...
		<script>
		var PostsLastReadingRef = new Firebase("https://csnoamproject.firebaseio.com/").child("PostsLastReading");
		...
		var pageLoadingDataDivs = $(".getLikes");	// posts has to be scanned
		...
		function scan_page_likes(){
			...
			if($('#moveon').is(':checked')){  			  	//if V is marked
				if(!PostsLastReading){					//if Firabase not loaded yet
					...
					$.get("data-post-likes.php?post_id="+page_id+"&max_pages=2", function(data){
